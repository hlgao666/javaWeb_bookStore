		...
		$(function () {
			$("button.addToCart").click(function () {
				...
				if(parseInt(stock)>0){
					...
					alert("The book you chose left 0");
				}
			});
		});
	</script>
		...
		<div id="book">
			...
			<div style="text-align: center">
				...
				<c:if test="${not empty sessionScope.cart.items}">
					...
					<div>
						<c:if test="${empty sessionScope.wrongMsg}">
							您刚刚将<span style="color: red" id="lastName">${sessionScope.lastName}</span>加入到了购物车中
						</c:if>
						<c:if test="${not empty sessionScope.wrongMsg}">
							<span style="color: red">${sessionScope.addItemWrongMsg}</span>
						</c:if>
					</div>
				</c:if>
